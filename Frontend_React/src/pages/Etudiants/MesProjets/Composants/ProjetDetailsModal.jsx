...
export default function ProjetDetailsModal({
  ...
  // Collaborateurs éligibles : étudiants et professeurs uniquement
  const availableUsers = allUsers.filter(
    (u) =>
      !collabIds.includes(u.id) &&
      (u.role === "etudiant" || u.role === "professeur"),
  );
  ...
  // Assignation tâches : uniquement les collaborateurs étudiants
  const assignableCollabs = (projet.collaborateurs || []).filter(
    (c) => c.user?.role === "etudiant",
  );
  ...
  const handleAddTache = async () => {
    ...
    await onCreateTache({
      ...
      projet: projet.id,
      assigne_a: newTache.assigne_a || assignableCollabs[0]?.user?.id,
              ...
              <>
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 mb-1">
                    Étudiants et enseignants uniquement
                  </p>
            ...
            {activeTab === "taches" && (
              <>
                <div>
                  ...
                  <p className="text-[10px] text-gray-400 mb-1">
                    Assignation réservée aux étudiants
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    ...
                    <select
                      ...
                      className="text-sm border border-gray-200 rounded-lg px-3 py-2 outline-none focus:border-indigo-500 transition-colors"
                    >
                      <option value="">Assigner à (étudiant)...</option>
